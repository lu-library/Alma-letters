	...
	<xsl:template match="/">
					...
					<div class="messageBody">
						...
						<xsl:choose>
							...
							<xsl:when test="(string-length($is_email_partner) > 0)">
								<xsl:message terminate="yes">user group is an EMAIL ILL PARTNER -
		TERMINATE </xsl:message>
							...
							<xsl:when test="(string-length($is_afn_patron) > 0)">
								<!-- handle AFN supported languages (is_preferred_lang_fr) defined
								in footer.xsl-->
                            <xsl:choose>
									...
									<table
											...
											<tr>
												<td>
													<!-- START AFN-VERSION 1.10 --> Le document suivant de [<xsl:call-template
		...
		peut être récupéré à <b>
											...
											<xsl:if
												...
												<tr>
													<td>
														<br /> La date d'expiration de l'exemplaire
		a été mise à jour. Ce dernier vous sera réservé jusqu'au <xsl:value-of
											...
											<!-- If notes exist, then we'll display the notes lable
											and the note -->
											<xsl:if
												test="notification_data/request/system_notes !='' ">
												<tr>
													<td>
														<br />
											...
											<tr>
												<td>
													<br />
													<!-- AFN-VERSION 1.1 --> Pour connaitre les heures de service et
		des informations liées à la récupération de documents veuillez consulter ci-dessus la page
		web de la bibliothèque. <br />
												</td>
											</tr>
									...
									<table cellspacing="0" cellpadding="5" border="0">
											<tr>
												<td>
													<!-- START AFN-VERSION 1.10 --> The following item from [<xsl:call-template
														name="AFNOrgName" />] - <xsl:value-of
														select="notification_data/phys_item_display/owning_library_name" />,
		which you requested on <xsl:value-of select="notification_data/request/create_date" /> can
		be picked up at <b>
														<xsl:value-of
															select="notification_data/request/assigned_unit_name" />
													</b>
											...
											<xsl:if
												...
												<tr>
													<td>
														<br /> The item expiry date has been updated
		and will be held for you until <xsl:value-of
															select="notification_data/request/work_flow_entity/expiration_date" />
													</td>
												</tr>
											</xsl:if>
											...
											<tr>
												<td>
													<br /> Please check the website at the pickup
		library indicated above for service hours and pickup information. <br />
												</td>
											</tr>
